...
if (fs.existsSync(hulhudhooSrc)) {
    ...
    files.forEach((f, i) => {
        ...
        hulhudhooPaths.push('/images/projects/hulhudhoo/' + newName);
    });
}
...
if (fs.existsSync(filadhooSrc)) {
    ...
    files.forEach((f, i) => {
        ...
        filadhooPaths.push('/images/projects/filadhoo/' + newName);
    });
}
...
function updateProject(content, titleSearch, heroImg, galleryArray) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.includes(titleSearch)) {
            foundTitle = true;
            result.push(line);
            result.push("      heroImage: '" + heroImg + "',");
            result.push("      galleryImages: " + JSON.stringify(galleryArray, null, 8) + ",");
