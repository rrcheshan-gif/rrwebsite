...
if (introRegex.test(content)) {
    content = content.replace(introRegex, newIntro.replace(/'/g, '"').replace(/"var\(--text-light\)"/g, '"var(--text-light)"')); // fix quotes
    // actually, let's just use double quotes directly in the HTML string representation by doing replace on the JS string.
