...
const splitMarkerStart = "{SERVICES.map((svc, idx) => (";
const splitMarkerEnd = "</Link>\n            ))}";

const startIndex = content.indexOf(splitMarkerStart);
const endIndex = content.indexOf(splitMarkerEnd);

if (startIndex !== -1 && endIndex !== -1) {
    const before = content.substring(0, startIndex);
    const after = content.substring(endIndex + splitMarkerEnd.length);
    ...
    console.log('Successfully replaced map logic using split.');
} else {
