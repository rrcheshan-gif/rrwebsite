...
export default function CIDARegistration() {
      ...
      >
        ...
        <div className="container" style={{ position: "relative", zIndex: 2 }}>
          ...
          <h1 style={{ color: "white", fontFamily: "var(--font-heading)", fontSize: isMobile ? "2.5rem" : "4.5rem", margin: 0, fontWeight: 800, textShadow: "0 4px 20px rgba(0,0,0,0.85)", textTransform: "uppercase" }}>
            CIDA <span style={{ color: "var(--primary-red)" }}>Registration</span>
          </h1>
          <p style={{ color: "#cbd5e1", fontSize: isMobile ? "1.1rem" : "1.3rem", marginTop: "15px", maxWidth: "700px", display: "inline-block", lineHeight: 1.6, fontWeight: 500 }}>
            Officially recognized and graded by the Construction Industry Development Authority of Sri Lanka.
          </p>
        </div>
      </section>
      ...
      <section style={{ padding: isMobile ? "40px 10px" : "60px 20px" }}>
        <div className="container" style={{ maxWidth: "1000px", margin: "0 auto", padding: "0 16px" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: "50px" }}>
             <ShieldCheck size={60} color="var(--primary-red)" style={{ margin: "0 auto 20px" }} />
             <h2 style={{ fontSize: isMobile ? "2rem" : "2.8rem", color: "var(--text-dark)", fontFamily: "var(--font-heading)", fontWeight: 800, margin: "0 0 10px 0" }}>Registration No: <span style={{ color: "var(--primary-red)" }}>{COMPANY_DATA.cidaRegistration}</span></h2>
             <p style={{ color: "var(--text-light)", fontSize: "1.1rem", maxWidth: "600px", margin: "0 auto" }}>
               Note: The following grading information is sourced from the company profile records and may require current verification.
             </p>
          </div>
          ...
          <div style={{ background: "var(--white)", padding: isMobile ? "30px 20px" : "50px", borderRadius: "24px", border: "1px solid var(--border-soft)", boxShadow: "0 10px 30px rgba(0,0,0,0.03)" }}>
            <h3 style={{ fontSize: "1.5rem", color: "var(--text-dark)", fontFamily: "var(--font-heading)", fontWeight: 800, marginBottom: "30px", borderBottom: "2px solid var(--border-soft)", paddingBottom: "15px" }}>Fields of Specialty & Grading</h3>
            
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: "20px" }}>
              {COMPANY_DATA.cidaGrades.map((item, idx) => (
                <div key={idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px", background: "var(--bg-light)", borderRadius: "12px", border: "1px solid var(--border-soft)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                    <CheckCircle2 size={20} color="var(--primary-red)" />
                    <span style={{ fontSize: "1.05rem", fontWeight: 600, color: "var(--text-dark)" }}>{item.field}</span>
                  </div>
                  <span style={{ background: "var(--primary-red)", color: "white", padding: "6px 12px", borderRadius: "8px", fontWeight: 700, fontSize: "1rem" }}>
                    {item.grade}
                  </span>
                </div>
              ))}
      ...
      <section style={{ padding: isMobile ? "60px 20px" : "100px 20px", textAlign: "center", background: "var(--white)", borderTop: "1px solid var(--border-soft)" }}>
        <div className="container" style={{ maxWidth: "800px", margin: "0 auto" }}>
          <h2 style={{ fontSize: isMobile ? "2.2rem" : "3.5rem", color: "var(--text-dark)", fontFamily: "var(--font-heading)", fontWeight: 800, marginBottom: "20px", lineHeight: 1.2 }}>
            View Our <span style={{ color: "var(--primary-red)" }}>Projects</span>
          </h2>
          <p style={{ color: "var(--text-light)", fontSize: "1.15rem", lineHeight: 1.7, marginBottom: "40px" }}>
            See how our certified capabilities translate into successful infrastructure development across Sri Lanka.
          </p>
