...
export async function POST(request: Request) {
  try {
    ...
    const mailOptions = {
      from: user,
      to: to,
      subject: `New Aggregates Order Request from ${name}`,
      html: `
        <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px;">
            <h2 style="color: #d32f2f; border-bottom: 2px solid #eee; padding-bottom: 10px;">New Aggregates Order Request</h2>
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <tr><td style="padding: 8px 0; border-bottom: 1px solid #eee;"><strong>Plant Location:</strong></td><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${plant || 'N/A'}</td></tr>
                ...
                <tr><td style="padding: 8px 0; border-bottom: 1px solid #eee;"><strong>Delivery Site:</strong></td><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${location || 'N/A'}</td></tr>
            </table>

            <h3 style="color: #555; margin-top: 20px;">Additional Instructions:</h3>
            <p style="background: #f9f9f9; padding: 15px; border-radius: 4px; border-left: 4px solid #d32f2f;">
                ${message ? message.replace(/\n/g, '<br>') : 'No additional instructions provided.'}
            </p>
        </div>
      `,
    };
